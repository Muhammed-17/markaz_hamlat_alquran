<x-layouts.markaz-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-[#0a5c36]">إدارة المعلمين</h1>
                <p class="text-gray-500 mt-1">{{ count($teachers) }} معلم مسجل في النظام</p>
            </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50">
                        ...
                        <td class="py-4 px-6 text-gray-600">
                            ...
                            <span class="bg-red-50 text-red-700 px-3 py-1 rounded-full text-xs font-semibold">معلم</span>
                            @elseif ($teacher->user->getRoleNames()->first() === 'supervisor')
                            <span class="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-xs font-semibold">مشرف</span>
                            @else
                            <span class="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full text-xs font-semibold">—</span>
                            @endif
                        </td>
                        ...
                        <td class="py-4 px-6">
                            <form action="{{ route('teachers.toggle', $teacher) }}" method="POST">
                                ...
                                <label class="inline-flex items-center cursor-pointer relative">
                                    <input type="checkbox" onchange="this.form.submit()" class="sr-only peer" {{ $teacher->user->status === 'active' ? 'checked' : '' }}>
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center justify-end gap-3">
                                ...
                                {{-- button delete --}}
                                <form method="POST" action="{{ route('teachers.destroy', $teacher) }}" class="text-red-400 hover:text-red-600 transition">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit">
