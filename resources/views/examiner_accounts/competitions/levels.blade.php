<x-layouts.markaz-layout>
    ...
    <nav class="text-xs text-gray-400 mb-4 flex items-center gap-1">
        ...
        <span>/</span>
        <a href="{{ route('examiner.competitions.index') }}" class="hover:text-[#0a5c36]">مسابقاتي</a>
        <span>/</span>
    ...
    @if($levels->isEmpty())
        <div class="bg-white rounded-2xl border border-gray-100 p-10 text-center text-gray-400">
            لا توجد مستويات مسندة إليك في هذه المسابقة.
        </div>
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            @foreach($levels as $row)
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <h3 class="font-bold text-gray-800 text-lg mb-4">{{ $row->competitionLevel->level?->name }}</h3>

                    <div class="grid grid-cols-2 gap-3 mb-5">
                        <div class="bg-gray-50 rounded-xl p-3 text-center">
                            <p class="text-xl font-black text-[#0a5c36]">{{ $row->participantsCount }}</p>
                            <p class="text-[11px] text-gray-500 mt-1">المشاركون</p>
                        </div>
                        <div class="bg-gray-50 rounded-xl p-3 text-center">
                            <p class="text-xl font-black text-emerald-600">{{ $row->completedCount }}</p>
                            <p class="text-[11px] text-gray-500 mt-1">مكتمل</p>
                        </div>
                    </div>

                    <a href="{{ route('examiner.participants.index', $row->competitionLevel) }}"
