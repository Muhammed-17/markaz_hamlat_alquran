...
'sort' => $field,
'dir' => request('sort') === $field && request('dir', 'desc') === 'asc' ? 'desc' : 'asc',
]);
...
<x-layouts.markaz-layout>
    <div dir="rtl"
        ...
        <div class="text-right w-full md:w-auto z-10">
            <h1 class="text-3xl font-black mb-2">إدارة المختبرين</h1>
            <p class="text-emerald-100/80 text-sm font-medium">
                @if(request()->anyFilled(['q']))
                {{ $examiners->total() }} نتيجة
                @else
                {{ $examiners->total() }} مختبر مسجل في النظام
                @endif
            </p>
        </div>

        <div class="flex flex-wrap items-center gap-4 w-full md:w-auto">
            @can('create examiners')
            <a href="{{ route('examiners.create') }}"
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-6">
        <form method="GET" action="{{ route('examiners.index') }}" class="flex flex-col lg:flex-row gap-4 items-end" dir="rtl">
            <div class="w-full lg:flex-1">
                <label for="filter_q" class="block text-xs font-bold text-gray-400 mb-1.5">البحث بالاسم أو الهاتف</label>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <table class="w-full text-right">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-sm font-bold text-gray-600 select-none">
                            <a href="{{ $sortLink('phone') }}" class="flex items-center gap-1 hover:text-gray-800">
                                <span>رقم الهاتف</span>
                                <span class="text-xs text-gray-400">{{ $sortIcon('phone') }}</span>
                            </a>
                        </th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-gray-600">
                            {{ \Illuminate\Support\Str::limit($examiner->address, 40) ?: '—' }}
                        </td>
                        <td class="px-6 py-4 text-gray-600 text-center">
                            {{ $examiner->competition_examiners_count }}
                        </td>
                        <td class="px-6 py-4 text-left">
                            <div class="flex items-center justify-end gap-3">
                                @can('view examiners')
                                <a href="{{ route('examiners.show', $examiner) }}"
                                    class="text-emerald-600 hover:text-emerald-800 transition" title="عرض المختبر">
                                ...
                                @can('edit examiners')
                                <a href="{{ route('examiners.edit', $examiner) }}" class="text-blue-500 hover:text-blue-700 transition">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                @can('delete examiners')
                                <form method="POST" action="{{ route('examiners.destroy', $examiner) }}"
                                    onsubmit="confirmDelete(event, { name: '{{ e($examiner->user?->name ?? 'المختبر') }}', type: 'المختبر' })"
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                            @if(request()->anyFilled(['q']))
                            لا يوجد مختبرون يطابقون البحث.
                            @else
                            لا يوجد مختبرون مسجلون حالياً.
                            @endif
                        </td>
                    </tr>
                    @endforelse
        ...
        <x-pagination :paginator="$examiners" />
    </div>
</x-layouts.markaz-layout>
