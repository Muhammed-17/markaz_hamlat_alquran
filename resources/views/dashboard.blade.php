<x-layouts.markaz-layout>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div class="bg-[#e0f2f1] text-[#00695c] px-4 py-2 rounded-lg font-medium flex items-center gap-2 self-start md:self-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        @if(Auth::user()->hasAnyRole(['admin', 'supervisor']))
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border-r-4 border-emerald-500 relative overflow-hidden">
                 ...
                 <div class="flex items-end gap-2">
                     <span class="text-3xl font-bold text-gray-800">{{ number_format($stats['monthly_revenue'] ?? 0) }}</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
             ...
             <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-wrap gap-4">
                @if(Auth::user()->hasAnyRole(['admin', 'supervisor']))
                    <a href="{{ route('students.index') }}" class="px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition">الطلاب</a>
                    <a href="{{ route('circles.index') }}" class="px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition">الحلقات</a>
                    <a href="{{ route('attendance.index') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">الحضور</a>
                @endif
             </div>
        </div>
    </div>
