<x-layouts.markaz-layout>
    <div class="max-w-4xl mx-auto space-y-6">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-[#0a5c36] mb-2">تعديل المشارك: {{ $externalParticipant->name }}</h1>
            <p class="text-gray-600">قم بتعديل بيانات المشارك الخارجي أدناه.</p>
        </div>

        <form action="{{ route('external-participants.update', $externalParticipant) }}" method="POST">
            @csrf
            @method('PUT')
            @include('external-participants.form')
