...
<x-layouts.markaz-layout>
    <div dir="rtl"
        ...
        <div class="text-right w-full md:w-auto z-10">
            <h1 class="text-3xl font-black mb-2">إدارة المستويات</h1>
            <p class="text-emerald-100/80 text-sm font-medium">
                @if(request()->anyFilled(['q']))
                {{ $levels->total() }} نتيجة
                @else
                {{ $levels->total() }} مستوى مسجل في النظام
                @endif
            </p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-6">
        <form method="GET" action="{{ route('levels.index') }}" class="flex flex-col lg:flex-row gap-4 items-end" dir="rtl">
            ...
            <table class="w-full text-right">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-sm font-bold text-gray-600">الوصف</th>
                        <th class="px-6 py-4 text-sm font-bold text-gray-600 text-center">عدد المسابقات المرتبطة</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-gray-600 text-center">
                            {{ $level->competition_levels_count }}
                        </td>
                        <td class="px-6 py-4 text-left">
                            <div class="flex items-center justify-end gap-3">
                                ...
                                <form method="POST" action="{{ route('levels.destroy', $level) }}"
                                    onsubmit="confirmDelete(event, { name: '{{ e($level->name) }}', type: 'المستوى' })"
