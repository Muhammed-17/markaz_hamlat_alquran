<x-layouts.markaz-layout>
    <div class="max-w-4xl mx-auto space-y-6">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-[#0a5c36] mb-2">تعديل المسابقة: {{ $competition->name }}</h1>
            <p class="text-gray-600">قم بتعديل بيانات المسابقة أدناه.</p>
        </div>

        <form action="{{ route('competitions.update', $competition) }}" method="POST">
            @csrf
            @method('PUT')
            @include('competitions.form')
